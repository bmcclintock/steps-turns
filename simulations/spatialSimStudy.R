...
for(sc in 1:length(scenarios)){
  ...
  for(isim in 1:nsims){
    ...
    UD1 <- Rhabit::getUD(covariates=covlist[[isim]], beta=delta1[sc,],log=TRUE)
    UD2 <- Rhabit::getUD(covariates=covlist[[isim]], beta=delta2[sc,],log=TRUE)
    ...
    results[[isim]][[1]]$AIC <- AIC(fit)
    results[[isim]][[1]]$pseudoRes <- pseudoRes(fit)
    ...
    results[[isim]][[2]]$pseudoRes <- pseudoRes(ST)
    ...
    results[[isim]][[3]]$pseudoRes <- pseudoRes(STcov)
    ...
    results[[isim]][[4]]$pseudoRes <- pseudoRes(STfull)
  ...
  save(beta0,delta1,delta2,beta1,beta2,results,sd_1,sd_2,file=paste0("simulations/spatialSimStudy_",scenarios[sc],".RData"))
}
