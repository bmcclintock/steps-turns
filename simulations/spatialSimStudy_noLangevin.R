...
spatialStudyplotDens_noLangevin <- list()
spatialStudyplotDens_noLangevin[[sc]] <- list()
...
for(isim in 1:nsims){
  ...
  DM2 <- list(mu=list(mean.x=~0+mu.x_tm1+crw(mu.x_tm1)+cov1.x+cov2.x+cov3.x+d2c.x,
                      mean.y=~0+mu.y_tm1+crw(mu.y_tm1)+cov1.y+cov2.y+cov3.y+d2c.y,
  ...
  spatialStudyplotDens_noLangevin[[sc]][[isim]] <- plotDens(origData[[isim]],sc)
  
  ST <- suppressMessages(fitHMM(origData[[isim]],nbStates=2,dist=list(step="gamma",angle="vm"),Par0=list(step=c(1.7,3,1,1.3),angle=c(0.01,0.5)),formula=formula,beta0=beta0,delta0=c(0.5,0.5),optMethod="TMB",control=list(silent=TRUE,maxit=10000,eval.max=10000),fixPar=list(delta=c(NA,NA))))
  if(ST$mle$step[1,1]>ST$mle$step[1,2]){
    swpar <- getPar0(ST,stateNames=c("state 2","state 1"))
    ST <- suppressMessages(fitHMM(origData[[isim]],nbStates=2,dist=list(step="gamma",angle="vm"),Par0=swpar$Par,formula=formula,beta0=swpar$beta,delta0=c(0.5,0.5),optMethod="TMB",control=list(silent=TRUE,maxit=10000,eval.max=10000),fixPar=list(delta=c(NA,NA))))
  }
  results[[isim]][[2]] <- list()
  ...
  if(STcov$conditions$optMethod=="TMB") {
    results[[isim]][[3]]$mod <- STcov$mod$out
...
save(beta0,beta1,beta2,crw1,crw2,results,sd_1,sd_2,file=paste0("simulations/spatialSimStudy_",sc,"_noLangevin.RData"))
save(spatialStudyplotDens_noLangevin,file="simulations/spatialStudyplotDens_noLangevin.RData")


spatial_noLangevinplot <- list()
spatial_noLangevinplot$viterbi <- spatial_noLangevinplot$beta12 <- spatial_noLangevinplot$beta21 <- list()
for(sc in c("low","moderate","high")){
  load(paste0("simulations/spatialSimStudy_",sc,"_noLangevin.RData"))
  sdf <- data.frame(model=factor(rep(c("pot","ST","STcov","STfull"),each=nsims)))
  ...
  spatial_noLangevinplot$viterbi[[sc]] <- ggplot(sdf, aes(x=model, y=viterbi)) + ylab("Proportion correct state assignments")+
       geom_violin(trim=TRUE,fill="grey")+ geom_boxplot(width=0.1)+geom_hline(yintercept=0.95,linetype=2,col=2)+ scale_y_continuous(limits = c(0.3,1)) 
  covNames <- c("cov1","cov2","cov3","d2c")
  spatial_noLangevinplot$beta12[[sc]] <- spatial_noLangevinplot$beta21[[sc]] <- list()
  for(j in 1:length(covNames)){
    ...
    spatial_noLangevinplot$beta12[[sc]][[cov]] <- ggplot(sdf, aes(x=model, y=!!sym(paste0("beta12",cov)))) + ylab(expression(beta[12]))+
      geom_violin(trim=FALSE,fill="grey")+ geom_boxplot(width=0.1)+geom_hline(yintercept=beta0[1+j],linetype=2,col=2)+ scale_y_continuous(limits = c(-2,2)) 
    #vioplot::vioplot(as.formula(paste0("beta21",cov,"~model")),data=sdf)
    #abline(h=beta0[6+j],col=2,lty=2)
    spatial_noLangevinplot$beta21[[sc]][[cov]] <- ggplot(sdf, aes(x=model, y=!!sym(paste0("beta21",cov)))) + ylab(expression(beta[21]))+
      geom_violin(trim=FALSE,fill="grey")+ geom_boxplot(width=0.1)+geom_hline(yintercept=beta0[6+j],linetype=2,col=2)+ scale_y_continuous(limits = c(-2,2)) 
  }
}
save(spatial_noLangevinplot,file="simulations/spatialSimStudyplots_noLangevin.RData")
