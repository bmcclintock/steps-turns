...
for(sc in 1:length(scenarios)){
  ...
  results_list <- foreach(isim = 1:nsims) %dorng% {
    ...
    spatialStudyplotDens_noLangevin <- plotDens(origData,sc)
    ...
    list(
      ...
      spatial_dens = spatialStudyplotDens_noLangevin, 
      orig_data = origData
    )
  }
  save(beta0,delta1,delta2,beta1,beta2,results_list,sd_1,sd_2,file=paste0("simulations/spatialSimStudy_",scenarios[sc],"_noLangevin.RData"))
}


spatial_noLangevinplot <- spatialStudyplotDens_noLangevin <- list()
...
for(sc in scenarios){
  ...
  spatialStudyplotDens_noLangevin[[sc]] <- results_list[[100]]$spatial_dens
